    ...
    <AriaColorSwatchPickerItem {...props} className={itemStyles}>
      {({ isSelected }) => (
        <>
          <ColorSwatch />
          {isSelected && (
            <div className="absolute top-0 left-0 w-full h-full border-2 border-black dark:border-white  outline-2 outline-white dark:outline-black -outline-offset-4 rounded-xs forced-color-adjust-none" />
          )}
        </>
      )}
